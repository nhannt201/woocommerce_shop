 ...
 * Access to this singleton is via global $barter_a13
 */
class Barter_Framework
{
     ...
     * kind of constructor
     */
    function barter_start()
    {
        /**
         * Define bunch of helpful paths and settings
         */
        define('BARTER_TPL_SLUG', 'barter');//it is not always same as directory of theme
        define('BARTER_OPTIONS_NAME_PART', 'Barter');
        define('BARTER_THEME_ID_NUMBER', '66');
        define('BARTER_OPTIONS_NAME', 'skt13_option_barter');
        define('BARTER_CACHE', 'skt13_barter_cache');
        define('BARTER_THEME_VERSION', '1.0');
        define('BARTER_THEME_VER', BARTER_THEME_VERSION ); //legacy - do not use
        define('BARTER_MIN_COMPANION_VERSION', '1.0');
        define('BARTER_MIN_PHP_VERSION', '5.6');
        define('BARTER_MIN_WP_VERSION', '5.0');

        //theme root
        define('BARTER_TPL_URI', get_template_directory_uri());

        //plugins recommended by theme
        define('BARTER_TPL_PLUGINS', BARTER_TPL_URI . '/advance/plugins');
        define('BARTER_TPL_PLUGINS_DIR', get_template_directory() . '/advance/plugins');

        //misc theme globals
        define('BARTER_INPUT_PREFIX', 'a13_');
        define('BARTER_CONTENT_WIDTH', 800);

        //check minimal requirements for WordPress and PHP
        if (
            version_compare( $GLOBALS['wp_version'], BARTER_MIN_WP_VERSION, '<' )
            ||
            version_compare( PHP_VERSION, BARTER_MIN_PHP_VERSION, '<' )
        ) {
        ...
        if( is_customize_preview() ){
            ...
            add_action( 'wp_loaded', array( $this, 'barter_customizer_wp_loaded' ) );
            //before save
            add_action('customize_save', array($this, 'barter_customizer_customize_save_before'));
            //perform option save while using customizer
            add_action('customize_save_after', array($this, 'barter_customizer_customize_save_after'));
        }
        ...
        if ( is_admin() ) {
            ...
            /** @noinspection PhpIncludeInspection */
			get_template_part('advance/admin/barterinfo', 'pages');
            /** @noinspection PhpIncludeInspection */
			
			get_template_part('advance/admin/barterinfo-pages-functions');
            ...
            if (current_user_can('update_core')) {
                add_action( 'admin_notices', array(&$this, 'barter_check_for_warnings') );
            }
        }
       ...
       $file_to_test = get_template_part('advance/barter');  
        if( is_file( $file_to_test ) ){
            /** @noinspection PhpIncludeInspection */
            require_once $file_to_test;
        }
        ...
        add_action('after_setup_theme', array(&$this, 'barter_setup'));
    }
     ...
     */
    function barter_set_sections($section){
        /**
         * @since 2.3.0
         */
        do_action( 'barter_options_before_'.$section['id'] );
         ...
         * @since 2.3.0
         */
        do_action( 'barter_options_after_'.$section['id'] );
    }
     ...
     * @return array
     */
    function barter_set_settings_set($set, $values){
     ...
     * @since 2.3.0
     */
    function barter_get_settings_set($set){
     ...
     */
    function barter_customizer_wp_loaded() {
        $this->theme_options = get_option(BARTER_OPTIONS_NAME);
        $this->load_options();
    }
     ...
     */
    function barter_customizer_customize_save_before()
    {
     ...
     */
    function barter_customizer_customize_save_after()
    {
        //get new set of options
        $this->theme_options = get_option(BARTER_OPTIONS_NAME);
        $this->load_options();

        do_action( 'barter_generate_user_css' );

        //refresh cache
        delete_option(BARTER_CACHE);
    }

    /**
     * Various setup actions for setting up theme for WordPress
     */
    function barter_setup()
    {
        global $content_width;
        //content width
        if (!isset($content_width)) {
            $content_width = BARTER_CONTENT_WIDTH;
        }
        ...
        if (
            ...
            ( function_exists('barter-a13fe_user_css_name') && ! file_exists( barter-a13fe_user_css_name() ) ) ||
        ...
        //LANGUAGE
        load_theme_textdomain( 'barter', get_theme_file_path( 'languages' ) );
        ...
        register_nav_menus(array(
            'header-menu' => __( 'Site Navigation', 'barter' ),
        ));
    }
     ...
     */
    function barter_check_for_warnings()
    {
        $notices = array();
        $valid_tags = array(
            'a' => array(
        ...
        foreach ($notices as $id => $notice) {
            ...
            if( !barter_is_admin_notice_active($id) ){
                continue;
            }
        ...
        do_action( 'barter_theme_notices' );
    }
    ...
    public function set_options( $overload_options = array() )
    {
        if( is_array($overload_options) && count($overload_options) > 0){
            update_option(BARTER_OPTIONS_NAME, $overload_options);
    ...
    public function get_option($index, $default = '', $filter = true)
    {
        ...
        return apply_filters('barter_a13_options_'.$index, $option_to_return );
    }
    ...
    public function get_option_color_rgba( $index )
    {
        ...
        if ( is_array( $option ) ) {
            if ( isset( $option['rgba'] ) ) {
                ...
                return barter_hex2rgba( $option['color'], $option['alpha'] ); //we got RGBA
            } else {
                return ''; //empty string as it is probably not set yet
            }
        }
...
//            'images_manager' //no parent options here
        ...
        foreach ($option_func as $function) {
            $function_to_call = 'barter_meta_boxes_' . $function;
    ...
    public function get_meta_required_array() {
        ...
        if(strlen($post_type)){
            switch ( $post_type ) {
                case 'post':
                    $meta_boxes = barter_meta_boxes_post();
                    break;
                case 'page':
                    $meta_boxes = barter_meta_boxes_page();
                    break;
            }
    ...
    public function prepare_theme_vars(){
        ...
        elseif( $cache === false || $cache['version'] !== BARTER_THEME_VERSION || defined( 'ICL_SITEPRESS_VERSION') || defined( 'POLYLANG_BASENAME' ) ){
            ...
            //normal flow
                ...
                $cache = array(
                    ...
                    'version' => BARTER_THEME_VERSION
                );

                //save cache
                update_option( BARTER_CACHE, $cache );
            }
        }
        //cache hit
        else{
     ...
     */
    function barter_get_meta($field, $id = false)
    {
        $family = '';

        if (!$id && barter_is_no_property_page()) {
     ...
     */
    function barter_get_social_icons_list($what = 'names'){
        $icons = array(
        ...
        $icons = apply_filters('barter_social_icons_list', $icons );
        $result = array();
    ...
    function barter_get_standard_fonts_list(){
        return array(
    ...
    function barter_is_companion_plugin_ready($fail_message = false, $silent = false){
        ...
        if ( empty( $plugins[$plugin_file] ) ) {
            ...
            $label = __( 'Install', 'barter' ) . ' : ' . __( 'SKT Templates – Elementor & Gutenberg templates', 'barter' );
        }
        ...
        elseif ( is_plugin_inactive( $plugin_path ) ){
            ...
            $label = __( 'Activate', 'barter' ) . ' : ' . __( 'SKT Templates – Elementor & Gutenberg templates', 'barter' );
        }

        //not up to date
        elseif( version_compare( $plugins[$plugin_file]['Version'], BARTER_MIN_COMPANION_VERSION, '<'  ) ){
            $ready = false;
            wp_enqueue_script( 'updates' );
            $classes =  ' update-now';
            ...
            $label = __( 'Update', 'barter' ) . ' : ' . __( 'SKT Templates – Elementor & Gutenberg templates', 'barter' );
        }

        if(!$ready && !$silent){
            $message = $fail_message ? $fail_message : __( 'This feature requires SKT Templates plugin to be active and in the proper version.', 'barter' );
