<?php
if(!function_exists( 'barter_cpt_meta_fields' )){
	/**
	 * Return internet address and custom fields of post
	 *
	 * @return string
	 */
    function barter_cpt_meta_fields(){
        $fields = '';
	    $custom_fields_number = apply_filters('barter_custom_fields_number', 5);
        ...
        for($i = 0; $i <= $custom_fields_number; $i++){
	        if( $i === 0 ) {
		        ...
		        if ( strlen( $temp ) ) {
			        $temp = esc_html__( 'Website', 'barter' ).':'.$temp;
		        }
	        }
	        else{
		        //custom fields
		...
		if(strlen($fields)){
			$html =
				'<div class="meta-data">'
			       ...
			       .apply_filters('barter_after_cpt_meta_fields','').
		        '</div>';
		}
...
if(!function_exists( 'barter_cpt_meta_data' )){
	/**
	 ...
	 * @return string
	 */
    function barter_cpt_meta_data( $content ){
	    return $content . barter_cpt_meta_fields();
    }
}


if(!function_exists( 'barter_cpt_social' )){
	/**
	 ...
	 *
	 */
    function barter_cpt_social( $link, $title ){
 ...
 */
function barter_get_gallery_item_images($item, $collector, $columns, $bricks_max_width, $brick_margin, $brick_proportion = 0){
	global $barter_a13;
	...
	$quality = (int)$barter_a13->get_option( 'a13ir_image_quality' );
...
if(!function_exists('barter_cpt_list_navigation')){
	/**
	 * Displays page navigation for custom post types
	 *
	 * @param bool|WP_Query $query
	 */
	function barter_cpt_list_navigation( $query = false ){
		if($query === false){
			global $wp_query;
			$query = $wp_query;
		}
		...
		$links = paginate_links( apply_filters( 'barter_pagination_args', array(
			'add_args'     => '',
...
if(!function_exists('barter_loop_pagination')){
	/**
	 ...
	 * @return array
	 */
	function barter_loop_pagination($args){
		$args['type'] = 'plain';
		$args['prev_text'] = _x( 'Previous', 'previous product', 'barter' );
		$args['next_text'] = _x( 'Next', 'next product', 'barter' );

		return $args;
	}
}
add_filter('barter_pagination_args', 'barter_loop_pagination' );


if(!function_exists('barter_make_media_collection')){
	/**
	 ...
	 */
    function barter_make_media_collection($id = null, $args = array()){
	    if(is_null($id)){
	        $id = get_the_ID();
	    }
	    ...
	    <ul class="gallery-media-collection screen-reader-text">
			<?php echo barter_prepare_frontend_gallery_html( barter_prepare_gallery_attachments( $value, $id ), $order, $id, $args ); ?>
		</ul>
	    <?php
    }
}
 ...
 * @return array
 */
function barter_prepare_gallery_attachments( $value, $post_id = null ){
	global $barter_a13;
	...
	if ( ! empty( $value ) ) {
		...
		$proofing_enabled = $barter_a13->barter_get_meta( '_proofing', $post_id ) === 'on';
		...
		if ( $media_count ) {
			...
			barter_prepare_external_media( $externals );
 ...
 */
function barter_prepare_external_media(&$items){
	global $barter_a13;

	$proofing_enabled = $barter_a13->barter_get_meta( '_proofing' ) === 'on';
 ...
 */
function barter_prepare_frontend_gallery_html( $attachments, $order, $id, $args ){
	if( $order === 'DESC' ){
		...
		foreach ( $attachments as $item ) {
			...
			elseif( is_array($item) && $item['type'] === 'audio' ){
				...
				if ( $item['id'] === 'external' ) {
					...
					//video details
					$temp =  barter_detect_movie( $item['videolink_link'] );
					...
					$collector[ 'video_player' ]   = barter_get_movie_link( $temp );       //Video API address
					...
					list(
						...
						$collector[ 'thumb' ]
						) = barter_get_gallery_item_images($item, $collector, $columns, $bricks_max_width, $brick_margin, $brick_proportion);
			...
			barter_frontend_gallery_item_html( $collector, $id, $args );
 ...
 * @param array $args gallery arguments
 */
function barter_frontend_gallery_item_html( $collector, $post_id, $args ){
	global $barter_a13;
	static $id = 0;
	$id++;
	$proofing_enabled = $args['proofing'] === 'on';
	...
	if(strlen($collector[ 'filter_tags' ])){
		$album_filters = barter_get_media_filters($post_id);
		$item_filters = array();
		$data_filter = '';
	...
	<li class="<?php echo esc_attr($collector[ 'item_class' ]); ?>"<?php echo $data_attr;//escaped while creating above ?>>
		...
		if( $collector[ 'attachment_type' ] === 'internal' && $collector[ 'type' ] === 'video' ){
			...
			//barter_video() produces escaped content
			echo '<div class="album-video" id="album-video-'.esc_attr( $id ).'">'.barter_video( $video_attr ).'</div>';
		}
		...
		if ( !$proofing_enabled ) {
			if(function_exists( 'ADDTOANY_SHARE_SAVE_KIT' )){
				...
				if ( $barter_a13->get_option( 'album_social_icons' ) === 'on' && $sharing_enabled ){

					$back_to_post = $barter_a13->get_option( 'album_share_type' ) === 'album';
					//share link will link to album and open image in lightbox
					if($back_to_post){
						$album_url = barter_current_url();
...
//				    ADDTOANY_SHARE_SAVE_KIT( array( 'linkname' => $collector['title'], 'linkurl' => $collector['src'] ) );
		...
		//add to cart button
		if( barter_is_woocommerce_activated() && isset( $collector['product_id'] ) && $collector[ 'product_id' ]){
 ...
 */
function barter_calculate_height_proportion($brick_proportion){
	$height_proportion = 0;
	//prepare proportion from string
 ...
 */
function barter_get_media_filters($id = null){
...
if ( ! function_exists( 'barter_cpt_as_frontpage_title_fix' ) ) {
	/**
	 ...
	 */
	function barter_cpt_as_frontpage_title_fix( $title ) {
...
if ( ! function_exists( 'barter_cpt_as_frontpage_menu_fix' ) ) {
	/**
	 ...
	 */
	function barter_cpt_as_frontpage_menu_fix( $classes, $item ) {
