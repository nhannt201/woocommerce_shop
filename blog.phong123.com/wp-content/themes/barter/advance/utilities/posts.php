...
if(!function_exists( 'barter_get_post_meta_data' )){
    /**
     * Returns post meta data like date, author, comments number and categories
     *
     * @since 2.3.0
     */
    function barter_get_post_meta_data() {
        global $barter_a13;

        $types      = barter_what_page_type_is_it();
	    ...
	    //return date
        if(
            ($post && $barter_a13->get_option( 'post_date') === 'on')
            ||
            ($post_list && $barter_a13->get_option( 'blog_date') === 'on')
        ){
            $return .= barter_posted_on();
        }

	    //return updated date
        if(
            ($post && $barter_a13->get_option( 'post_date') === 'updated')
            ||
            ($post_list && $barter_a13->get_option( 'blog_date') === 'updated')
        ){
            $return .= esc_html__( 'Updated: ', 'barter' ).barter_modified_on();
        }
        ...
        elseif(
		    ($post && $barter_a13->get_option( 'post_author') === 'on')
		    ||
		    ($post_list && $barter_a13->get_option( 'blog_author') === 'on')
	    ){
		    $return .= barter_posted_by_author();
	    }

        //return comments number
        if(
            ($post && $barter_a13->get_option( 'post_comments') === 'on')
            ||
            ($post_list && $barter_a13->get_option( 'blog_comments') === 'on')
        ){
            $return .= barter_post_comments();
        }

        //Categories
        if(
            ($post && $barter_a13->get_option( 'post_cats') === 'on')
            ||
            ($post_list && $barter_a13->get_option( 'blog_cats') === 'on')
        ){
...
if(!function_exists( 'barter_post_meta_data' )){
    /**
     * Outputs post meta data like date, author, comments number and categories
     *
     * @since 2.3.0
     */
    function barter_post_meta_data() {
        echo barter_get_post_meta_data();
    }
}


if(!function_exists('barter_posted_on')){
	/**
	 * get HTML for date of post
     * @return string
     */
    function barter_posted_on() {
        return '<time class="entry-date published updated" datetime="'.esc_attr( get_the_date( 'c' ) ).'" itemprop="datePublished">'.get_the_date().'</time> ';
    }
}


if(!function_exists('barter_modified_on')){
	/**
	 * get HTML for date of post
     * @return string
     */
    function barter_modified_on() {
...
if(!function_exists('barter_posted_by_author')){
	/**
	 * Author of post
	 * @return string
	 */
	function barter_posted_by_author() {
        /** @noinspection HtmlUnknownAttribute */
        return
            /* translators: %s - author name */
			sprintf(  esc_html__( 'by %s ', 'barter' ),
                sprintf('<a class="vcard author" href="%1$s" title="%2$s" %4$s><span class="fn" %5$s>%3$s</span></a>',
				    esc_url(get_author_posts_url( get_the_author_meta( 'ID' ) )),
                    /* translators: %s - author name */
				    sprintf( esc_attr__( 'View all posts by %s', 'barter' ), get_the_author() ),
				    get_the_author(),
                    barter_get_schema_args('author'),
                    barter_get_schema_args('name')
                )
...
if(!function_exists('barter_post_comments')){
	/**
	 * comments link
	 *
     * @return string HTML
     */
    function barter_post_comments() {
...
if(!function_exists('barter_under_post_content')){
    function barter_under_post_content() {
        global $barter_a13;
        ...
        wp_link_pages( array(
                'before' => '<div id="page-links"><span class="page-links-title">'. esc_html__( 'Pages: ', 'barter' ).'</span>',
                'after'  => '</div>'
	        )
        );
        ...
        //Tags under content
        if(
            ($post && $barter_a13->get_option( 'post_tags' ) === 'on')
            ||
            ($post_list && $barter_a13->get_option( 'blog_tags' ) === 'on')
        ){
...
if(!function_exists('barter_author_info')){
     ...
     */
    function barter_author_info() {
        global $barter_a13;

        $author_description =  get_the_author_meta( 'description' );

        if( ( strlen($author_description) > 0 ) && $barter_a13->get_option( 'post_author_info' ) === 'on'): ?>
            <div class="about-author comment"<?php barter_schema_args('author_box'); ?>>
                <div class="comment-body">
                    <?php $author_ID = get_the_author_meta( 'ID' );
                        echo '<a href="'.esc_url( get_author_posts_url($author_ID) ).'" class="avatar"'.barter_get_schema_args('url').'>'.get_avatar( $author_ID, 90 ).'</a>';
                        echo '<strong class="comment-author"><span class="author-name" '.barter_get_schema_args('name').'>'.get_the_author().'</span>';
...
if(!function_exists('barter_posts_navigation')){
     ...
     */
    function barter_posts_navigation() {
        global $barter_a13;

        if($barter_a13->get_option( 'post_navigation' ) === 'on'){
            //posts navigation
            ...
            if($is_prev || $is_next){
                ...
                add_filter( 'barter_post_navigation_image_size', function(){
                    return array(245,245);
                });
                 *
                 */
                $image_sizes = apply_filters( 'barter_post_navigation_image_size', array( 245, 100 ) );

                if($is_prev){
                    $id = $prev_post->ID;
                    echo '<a href="'.esc_url( get_permalink($id) ).'" class="item prev">'
                         .'<span><i class="fa fa-long-arrow-'.( is_rtl() ? 'right' : 'left' ).'"></i> '.esc_html__( 'Previous article', 'barter' ).'</span>'
                         .'<span class="title">'.esc_html( $prev_post->post_title ).'</span>'
                         .'<span class="image">'.barter_make_post_image( $id, $image_sizes ).'</span>'
                         .'</a>';
                }
                if($is_next){
                    $id = $next_post->ID;
                    echo '<a href="'.esc_url( get_permalink($id) ).'" class="item next">'
                         .'<span>'.esc_html__( 'Next article', 'barter' ).' <i class="fa fa-long-arrow-'.( is_rtl() ? 'left' : 'right' ).'"></i></span>'
...
if(!function_exists('barter_excerpt_length')){
    /**
     ...
     */
    function barter_excerpt_length($length) {
        ...
        global $barter_a13;
        return intval( $barter_a13->get_option( 'blog_excerpt_length') );
    }
}
add_filter( 'excerpt_length', 'barter_excerpt_length' );


if(!function_exists('barter_new_excerpt_more')){
    /**
     ...
     * @return string HTML
     */
    function barter_new_excerpt_more($link) {
        $ajax_for_vc_grid = defined( 'DOING_AJAX' ) && DOING_AJAX && isset($_POST['action']) && ($_POST['action'] === 'vc_get_vc_grid_data');
        ...
        global $post, $barter_a13;
        if( $barter_a13->get_option( 'blog_read_more', 'on' ) === 'off' ){
            return '';
        }

        //return read more
        return '&hellip;
 <p> <a class="more-link" href="'. esc_url( get_permalink($post->ID) ) . '">'.esc_html__( 'Read more', 'barter' ).'</a></p>';
    }
}
add_filter( 'excerpt_more', 'barter_new_excerpt_more' );



if(!function_exists('barter_has_excerpt_read_more')){
	/**
	 * Adds read more when excerpt is provided by user
	 *
     * @return string HTML
     */
    function barter_has_excerpt_read_more($content) {
        global $post, $barter_a13;

        if( $barter_a13->get_option( 'blog_read_more', 'on' ) === 'on' && has_excerpt() ){
            $content .= '<p> <a class="more-link" href="'. esc_url( get_permalink($post->ID) ) . '">'.esc_html__( 'Read more', 'barter' ).'</a></p>';
        }

        return $content;
    }
}
add_filter( 'the_excerpt', 'barter_has_excerpt_read_more' );


if(!function_exists('barter_read_more_new_line')){
	/**
	 ...
	 * @return string HTML
	 */
    function barter_read_more_new_line($link) {
        return '<p>'.$link.'</p>';
    }
}
add_filter( 'the_content_more_link', 'barter_read_more_new_line' );



if(!function_exists('barter_comments_navigation')){
	/**
     * Comments navigation
     */
    function barter_comments_navigation() {
        if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
            ?>
            <nav class="navigation comment-navigation">
                <h2 class="screen-reader-text"><?php esc_html_e( 'Comment navigation', 'barter' ); ?></h2>
                <div class="nav-links">
                    <?php
                    if ( $prev_link = get_previous_comments_link( esc_html__( 'Older Comments', 'barter' ) ) ) :
                        printf( '<div class="nav-previous">%s</div>', wp_kses_post( $prev_link ) );
                    endif;

                    if ( $next_link = get_next_comments_link( esc_html__( 'Newer Comments', 'barter' ) ) ) :
...
if(!function_exists('barter_daoon_chat_post')){
    /**
     ...
     * @return string
     */
    function barter_daoon_chat_post($content) {
...
if(!function_exists('barter_display_items_from_query_post_list')) {
     ...
     * @param array         $args
     */
    function barter_display_items_from_query_post_list($query = false, $args = array()){
        global $barter_a13;
        ...
        $default_args = array(
            'columns'         => $barter_a13->get_option( 'blog_brick_columns' ),
            'max_width'       => $barter_a13->get_option( 'blog_bricks_max_width' ),
            'margin'          => $barter_a13->get_option( 'blog_brick_margin' ),
            'filter'          => false,
        ...
        if($args['filter']){
            ...
            barter_make_post_grid_filter($terms, 'posts-filter', $args['default_filter'], $args['all_filter']);
        }
        ...
        if ( ! $query->have_posts() ):
            ...
            <div class="formatter">
                <div class="real-content empty-blog">
                    <?php
                    if( !is_search() ){
                        echo '<p>'.esc_html__( 'Apologies, but no results were found for the requested archive.', 'barter' ).'</p>';
                    }
            ...
            $posts_classes = '';
            $posts_layout_class = $barter_a13->get_option( 'blog_post_look');
            ...
            if($post_list_page){
                ...
                $lazy_load_mode   = $barter_a13->get_option( 'blog_lazy_load_mode' );
            ...
            while ( $query->have_posts() ) : $query->the_post();
                ...
                //size of brick
                $brick_size = ($posts_layout_class === 'horizontal') ? 1 : $barter_a13->barter_get_meta( '_brick_ratio_x' );
                ...
                barter_schema_args('creative');
                echo '>';

                    if(post_password_required()){
                        barter_top_image_video($link_it, array_merge($args, array('page_type' => 'blog_type', 'post_grid' => true ) ) );
                        ?>
                        <div class="formatter">
                            <?php the_title('<h2 class="post-title entry-title"><a href="'. esc_url( get_permalink() ) . '"><span class="fa fa-lock"></span>', '</a></h2>'); ?>
                            <div class="real-content">
                                <p><?php esc_html_e( 'To view it please enter your password below', 'barter' ); ?></p>
                                <?php echo barter_password_form(); //escaped on creation  ?>
                            </div>
                        </div>
                        <?php
                    }
            ...
            echo '</div></div>';
            if(!$ajax_call){
                barter_post_list_individual_look($id, $args);
            }
        endif;
    }
}
 ...
 */
function barter_remove_hentry( $class ) {
    $class = array_diff( $class, array( 'hentry' ) );
    return $class;
}
add_filter( 'post_class', 'barter_remove_hentry' );



if ( ! function_exists( 'barter_next_posts_link_class' ) ) {
	/**
	 * Adding class for compatibility with Wp-paginate plugin + infinite scroll configuration
	 *
	 * @return string
	 */
	function barter_next_posts_link_class() {
		return 'class="next"';
	}
}

if ( ! function_exists( 'barter_prev_posts_link_class' ) ) {
	/**
	 * Adding class for compatibility with Wp-paginate plugin + infinite scroll configuration
	 *
	 * @return string
	 */
	function barter_prev_posts_link_class() {
		return 'class="prev"';
	}
}
add_filter( 'next_posts_link_attributes', 'barter_next_posts_link_class' );
add_filter( 'previous_posts_link_attributes', 'barter_prev_posts_link_class' );



if(!function_exists('barter_post_list_individual_look')){
    /**
     ...
     */
    function barter_post_list_individual_look($id, $args){
        $css = '';

        $max_width        = (int) $args['max_width'];
        $max_width        = esc_html( barter_make_css_rule( 'max-width', $max_width, '%spx' ) );
...
if(!function_exists('barter_get_schema_args')){
    /**
     ...
     */
    function barter_get_schema_args( $for ){
        $prop = '';
        $scope = false;
        $type = '';
        ...
        $attributes = apply_filters( 'barter_schema_args', $attributes, $for );
        /* SAMPLE USAGE */
        /*
        add_filter( 'barter_schema_args', function($args, $for){
                if($for == 'body'){
                    $args['type'] = 'Article';
                }
...
if(!function_exists('barter_schema_args')){
    /**
     ...
     * @return      string - string of HTML attributes
     */
    function barter_schema_args( $for ) {
        echo barter_get_schema_args( $for );
    }
}

if(!function_exists('barter_remove_wpseo')){
    /**
...
add_action( 'template_redirect', 'barter_remove_wpseo' );